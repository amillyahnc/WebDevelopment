...
for(let i = 0; 1 < meats.length; i+=1){
  build += `<div>`;
...
for(let i = 0; 1 < seafoods.length; i+=1){
  build += `<div>`;
  build += `   <h2> ${ seafood_titles[i] } </h2>`;
  build += `   <img src="images${seafoods[i]}"> `;
